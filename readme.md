...
| Layer             | Technology                                                                   |
| ----------------- | ----------------------------------------------------------------------------- |
| Face Detection    | OpenCV YuNet (DNN, per-frame) + RetinaFace re-alignment (recognition step)    |
...
├── .gitignore
├── screenshots/
├── models/                # YuNet ONNX weights — auto-downloaded on first run
...
```
> `models/`, `database/`, `alerts/`, and `eval_results/` aren't tracked in this repo — they're created automatically the first time you run the relevant script.
---
...
```
> **Note:** DeepFace will auto-download the ArcFace model weights on first run (~200 MB). If DeepFace isn't installed, the system automatically falls back to pixel-difference comparison. `requirements.txt` covers the live detection pipeline (`detect.py`). The same file also includes `scipy`, `scikit-learn`, and `matplotlib`, which are only needed if you plan to run `Evaluate.py` or experiment with `Sorttracker.py`.

> **Note:** On first run, `detect.py` will also auto-download the YuNet face-detection model (~340KB) into `models/`. If your environment has no internet access at runtime, manually download `face_detection_yunet_2023mar.onnx` from the [OpenCV Zoo](https://github.com/opencv/opencv_zoo/tree/main/models/face_detection_yunet) and place it at `models/face_detection_yunet_2023mar.onnx` beforehand.
...
[![Dashboard](https://github.com/pragnya3101/criminal-detection-cctv/raw/main/screenshots/dashboard.png)](/pragnya3101/realtime-face-recognition-dashboard/blob/main/pragnya3101/criminal-detection-cctv/blob/main/screenshots/dashboard.png)
...
| `YUNET_SCORE_THRESHOLD`| `0.7`   | Minimum YuNet confidence for a detection to count as a face             |
| `YUNET_NMS_THRESHOLD`  | `0.3`   | Non-max suppression overlap threshold for YuNet                         |

---

## 🧠 How It Works

1. **Face detection** — YuNet (lightweight ONNX face-detection DNN) scans each webcam frame for faces (`detect.detect_faces`)
...
used by `detect.py`**. The live pipeline uses the simpler centroid-distance `FaceTracker` instead. Wiring `Sort` in as the primary tracker (it would need an adapter to convert YuNet's
...
- **ArcFace embeddings are recomputed from images every time `detect.py` or `Evaluate.py` starts** — there's no on-disk caching, so startup time scales with database size.
...
- Detection accuracy still drops at extreme angles, very small/far faces, or heavy occlusion — better than Haar Cascade, but not perfect
...
**Pragnya Bodakuntla** B.Tech CSE(DS) | Hyderabad, India [GitHub](https://github.com/pragnya3101) · [LinkedIn](https://www.linkedin.com/in/pragnya-bodakuntla)
